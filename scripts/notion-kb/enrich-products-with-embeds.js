// Add embedded database views to product pages
...
async function enrichProductPage(productId, productName) {
  ...
  // Build content blocks with EMBEDS
  const blocks = [
    // Product description
    {
      type: 'callout',
    ...
    },
    {
      type: 'embed',
      embed: {
        url: `https://www.notion.so/${BLOG_DB_ID.replace(/-/g, '')}?v=${Date.now()}`,
      },
    },
    ...
    },
    {
      type: 'embed',
      embed: {
        url: `https://www.notion.so/${TECH_STACK_DB_ID.replace(/-/g, '')}?v=${Date.now()}`,
      },
    },
    ...
    },
    {
      type: 'embed',
      embed: {
        url: `https://www.notion.so/${API_DB_ID.replace(/-/g, '')}?v=${Date.now()}`,
      },
    },
    ...
    },
    {
      type: 'embed',
      embed: {
        url: `https://www.notion.so/${COMPONENTS_DB_ID.replace(/-/g, '')}?v=${Date.now()}`,
      },
    },
  ]

  // Get existing blocks and DELETE the yellow callout guides
  const existingBlocks = await callNotionAPI(`/blocks/${productId}/children`)

  console.log(`   🗑️  清除舊的指引 callouts...`)
  ...
  // Append new blocks with embeds
  console.log(`   ➕ 添加 embedded databases...`)
  await callNotionAPI(`/blocks/${productId}/children`, 'PATCH', { children: blocks })

  console.log(`   ✅ 已添加豐富內容（含 embedded databases）`)
}

async function main() {
  console.log('🎨 豐富化所有產品頁面（用 embeds）...\n')
  ...
  console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━')
  console.log('✅ 完成！所有產品頁面已豐富化（embedded databases）')
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━\n')
  console.log('💡 現在點進產品頁面應該可以看到 embedded database views 了！')
  console.log('   如果需要調整 filter，可以在 Notion 中手動設定')
}
